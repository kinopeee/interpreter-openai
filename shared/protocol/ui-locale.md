...
英語は日本語より長い。設定ウィンドウの固定サイズ（macOS 560×560、Windows 620×560）は初回は維持し、折り返しで収める。収まらない項目だけ幅/高さを最小限広げる。トレイの `NotifyIcon.Text` は **63 文字上限**。
...
- [ ] 実装 PR ではこの文書を更新せずに挙動を変えない。変えるなら文書を先に直す
...
- `ui.json` にユーザー向け枠文言があり、ソースに同等の直書きが残っていない（grep で確認）
- 表示言語 Picker があり、保存され、**再起動後だけ**反映される
- `system` / `ja` / `en` が仕様どおり
- 字幕本文・記録ファイル形式・prompt/keywords・ログ禁止が維持される
- Windows `dotnet test` と macOS `xcodebuild test`、`shared-contracts` が通る
- `VALIDATION.md` の表示言語項目がある
