...
internal static class UiCopy
{
    ...
    internal static void Install(UiLanguagePreference preference) =>
        UserCopy.InstallFromPreference(preference, CultureInfo.CurrentUICulture.TwoLetterISOLanguageName);
    ...
    internal static string Format(string key, string name, string value) =>
        UserCopy.Current.Format(key, new Dictionary<string, string>(StringComparer.Ordinal) { [name] = value });
    ...
    internal static string PairName(LanguagePair pair) =>
        pair switch
        {
    ...
    internal static string PresetName(string id) =>
        id switch
        {
    ...
    internal static string DelayName(RealtimeTranscriptionDelay delay) =>
        delay switch
        {
    ...
    internal static string NoiseName(RealtimeTranslationNoiseReduction noise) =>
        noise switch
        {
