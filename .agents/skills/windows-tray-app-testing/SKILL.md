...
Running `dotnet` from the repo root skips `windows/global.json` entirely, so the
probe would accept any 10.x SDK and the build could use one outside the policy.
