...
ビルド済みの `RealtimeTranslator-<tag>-macos-arm64.zip` を [Releases](https://github.com/kinopeee/interpreter-openai/releases) から取得できます。タグリリースの macOS 成果物は Developer ID Application 署名済みで、公証（notarization）と staple 済みです。SHA-256 は zip 内ではなく、同名の Release アセット `RealtimeTranslator-<tag>-macos-arm64.zip.sha256` です。起動前に検証し、`/Applications` へ移してください。
...
| 表示言語 | `システムと同じ` / `日本語` / `English`。保存は即時、反映はアプリ再起動後。翻訳方向（`languagePair`）とは独立です。 |
...
| 項目 | 説明 |
| --- | --- |
| ノイズ低減 | `近距離マイク`（`near_field`）または `遠距離マイク`（`far_field`、既定）。変更は次回の録音開始から反映されます。 |
| 認識遅延 | `gpt-live-transcribe` の `delay`。値を上げると短い発話の精度が上がり、字幕は遅くなります。既定は `低遅延`（`low`）。選択肢は最速 / 低遅延 / バランス / 高精度 / 最高精度。 |
| マイク音量の自動調整 | 既定で有効。話している間だけ音量を 100 ms 単位で調整し、雑音だけの区間では上げません。瞬間的な大きな音はその 100 ms だけ抑えます。変更は次回の録音開始から反映されます。 |
...
| 字幕位置 | メニューバーの「字幕位置を編集」でドラッグ移動し、編集 OFF で保存。複数ディスプレイではカーソルのある画面へ移る。 |
| 表示ディスプレイ | メニューバーの「字幕を表示するディスプレイ」で選ぶと、その画面の下端中央へ移動して保存。ミラーリング中は移せないため PowerPoint の発表者ツールを ON にする。ミラーリング中に位置編集を終えても保存位置は変わらない。 |
...
`v*` のタグを push すると `release` ワークフローがWindows（win-x64）とmacOS（arm64）の両方をテスト→ビルド→zip化し、同じReleaseへ添付します。macOS版は Developer ID 署名・公証・staple 済みです（手順は上記「ダウンロード（リリース）」と同じ）。
...
| 設定の保存先 | `%LOCALAPPDATA%\RealtimeTranslator\settings.json`（フォントサイズ、字幕位置、同意状態、認識プロンプト・キーワード・遅延・ノイズ低減・マイク音量の自動調整、表示言語）。 |

プロンプト・キーワード・認識遅延の変更は録音中でも数秒でセッションへ反映されます。ノイズ低減とマイク音量の自動調整の変更は次回の録音開始から反映されます。
