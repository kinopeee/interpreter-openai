...
1. 設定画面の言語ペア Picker で `ja-en`、`ja-es`、`en-es` を順に選択します。
2. 各ペアを選択した状態で設定を保存し、いったん録音を停止してから次の録音を開始します。開始後の原文認識・翻訳 target が選択したペアの2言語だけを使うことを確認します。
3. 録音中に言語ペアを別のペアへ変更しても、現在の録音では接続済みの2言語と routing が変わらないことを確認します。録音を停止して次の録音を開始した後にだけ新しいペアが反映されることを確認します。
4. `ja-es` で日本語の1文とスペイン語の1文をそれぞれ話し、原文と相手言語の訳文が同じ字幕ペアとして表示されることを確認します。
5. `en-es` で英語の1文とスペイン語の1文をそれぞれ話し、原文と相手言語の訳文が同じ字幕ペアとして表示されることを確認します。
6. `en-es` で短い相槌（`sí`、`yeah` など）を交互に話し、1回の短い発話だけで lane が往復せず、連続した反対言語の証拠が揃ったときだけ切り替わることを確認します。
7. `en-es` で固有名詞だけを話し、英語・スペイン語の排他的な証拠がない固有名詞で誤判定しないことを確認します。英語とスペイン語を1文の中で混ぜる code switching も話し、字幕表示と lane 切替が破綻しないことを確認します。
8. `ja-es` / `en-es` でアクセント付き文字（`á`、`é`、`í`、`ó`、`ú`、`ü`、`ñ`）と `¿`、`¡` を含む文を話し、字幕に正しく描画され、末尾切り詰め位置で文字や句読点が欠けたり不自然に分割されたりしないことを確認します。
9. language pair キーのない既存設定を用意して起動し、既定の `ja-en` で起動することを確認します。
10. ペアを変更して録音を開始し、ペアに対応する transcription prompt / keywords の既定値が適用されることを確認します。prompt または keywords をカスタム値へ変更してからペアを変更し、カスタム文言が保持されることを確認します。

### macOS GUI確認

- [ ] 設定画面の言語ペア Picker に `日本語 ↔ 英語`、`日本語 ↔ スペイン語`、`英語 ↔ スペイン語` が表示される
- [ ] メニューバー表示に現在選択中の言語ペアが反映される

### 表示言語（手動）

1. OS が日本語、`表示言語` が `システムと同じ` で起動し、メニュー・設定・待機バナーが日本語であること。ホットキー表記は `Control + Option + Space`。
2. 設定で English を選び、**再起動せず**にメニュー・設定・バナーがまだ日本語であること。ヒントが出ること。
3. 再起動後は英語 UI。翻訳ペアは変わらず、`ja-en` のまま日本語音声を英語字幕にできること。
4. 設定で 日本語 を選び再起動すると日本語に戻ること。
5. 設定を変更して再起動しても、翻訳ペア・APIキー・オーバーレイ位置が保持されること。
6. マイク許可ダイアログは OS の表示言語に従うこと（`NSMicrophoneUsageDescription` は `InfoPlist.strings`）。アプリが English でも日本語 OS なら日本語。仕様。
7. 字幕記録を書き出し、`原文:` / `訳文:` / `=== 録音開始` が英語 UI でも同じであること。
8. 無効 API キーでバナーがロケールされ、キー本文が無いこと。
9. 設定の英語表示でタブ・ボタンが切れないこと。
...
- [ ] ログ、status file（DEBUGのみ）、字幕バナー、クラッシュ情報にAPIキー・Authorization/Bearer・音声・原文・訳文がない
- [ ] `/tmp/realtimetranslator.status`を削除してからReleaseビルドを起動し、起動・録音開始・停止・終了後も作成されない
- [ ] エラー時にモーダルではなく字幕バナーへ表示され、秘密情報を含まない
...
1. 設定ウィンドウの言語ペア ComboBox で `ja-en`、`ja-es`、`en-es` を順に選択します。
2. 各ペアを選択した状態で設定を保存し、いったん録音を停止してから次の録音を開始します。開始後の原文認識・翻訳 target が選択したペアの2言語だけを使うことを確認します。
3. 録音中に言語ペアを別のペアへ変更しても、現在の録音では接続済みの2言語と routing が変わらないことを確認します。録音を停止して次の録音を開始した後にだけ新しいペアが反映されることを確認します。
4. `ja-es` で日本語の1文とスペイン語の1文をそれぞれ話し、原文と相手言語の訳文が同じ字幕ペアとして表示されることを確認します。
5. `en-es` で英語の1文とスペイン語の1文をそれぞれ話し、原文と相手言語の訳文が同じ字幕ペアとして表示されることを確認します。
...
- [ ] 設定ウィンドウの言語ペア ComboBox に `日本語 ↔ 英語`、`日本語 ↔ スペイン語`、`英語 ↔ スペイン語` のラベルが表示される
- [ ] トレイ表示に現在選択中の言語ペアが反映される
- [ ] 通常モードでオーバーレイの字幕本文をクリックすると背後アプリへ入力が届く（クリック透過）
...
1. OS が日本語、`表示言語` が `システムと同じ` で起動し、トレイ・設定・待機バナーが日本語であること。ホットキー表記は `Ctrl + Alt + Space`。
2. 設定で English を選び、**再起動せず**にトレイ・設定・バナーがまだ日本語であること。ヒントが出ること。
3. 再起動後は英語 UI。翻訳ペアは変わらず、`ja-en` のまま日本語音声を英語字幕にできること。
4. 設定で 日本語 を選び再起動すると日本語に戻ること。
5. 字幕記録を書き出し、`原文:` / `訳文:` / `=== 録音開始` が英語 UI でも同じであること。
6. 無効 API キーでバナー／通知がロケールされ、キー本文が無いこと。
7. 設定の英語表示でタブ・ボタンが切れないこと。トレイアイコンのツールチップが 63 文字以内であること。
