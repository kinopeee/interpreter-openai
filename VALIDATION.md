...
2. `表示言語` を `English` に変更し、設定画面のラベルはすぐ変わらず、再起動後に英語になること。
3. `表示言語` を `日本語` に固定し、OS を英語にしても UI が日本語のままであること。
4. 設定を変更して再起動しても、翻訳ペア・APIキー・オーバーレイ位置が保持されること。
5. マイク許可ダイアログは OS の表示言語に従うこと（`NSMicrophoneUsageDescription` は `InfoPlist.strings`）。
