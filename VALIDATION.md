...
3. 録音中にプロンプトを編集し、debounce（800 ms）が切れる前に設定ウィンドウを閉じて、`transcriptionPrompt` が `settings.json` へ反映され字幕が継続することを確認しました。セッションへの反映は `session.update` をUIから直接観測できないため間接判定です。
