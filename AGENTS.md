...
- `shared/fixtures/v<N>/` は両実装のバージョン付き契約正本。現行の subtitle と audio の契約は v2、その他は v1 とし、Swift テストと Windows 版の同値性を保つ。既存の subtitle v1 と audio v1 も保持し、`scripts/ci-shared-contracts.sh` で全バージョンを検査する。
